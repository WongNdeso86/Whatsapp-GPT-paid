...
// Buat array dengan nomor telepon pengguna yang diizinkan

//Bagian Koding ChatGPT
const { Configuration, OpenAIApi } = require("openai");
const configuration = new Configuration({
    apiKey: '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',
});
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (type === "notify" && !messages[0].key.fromMe) {
            try {
                ...
                const allowedUsers = ['6281234567890', '6281943338962@s.whatsapp.net'];

      // Cek apakah nomor pengirim termasuk dalam pengguna yang diizinkan
      if (allowedUsers.includes(senderNumber)) { // Jika termasuk pengguna yang diizinkan
        // Lakukan tindakan
      } else { // Jika tidak termasuk pengguna yang diizinkan
        await sock.sendMessage(
          senderNumber, 
          { text: "Anda belum mendaftar untuk menggunakan saya, hubungi developer @e.2f.endi" },
          { quoted: messages[0] },
          2000
        );
        return; // Hentikan eksekusi kode selanjutnya
                ...
                const isMessageMentionBot = incomingMessages.includes("@6287817187690");
                ...
                if (!isMessageFromGroup) {
                    ...
                    if (incomingMessages.includes('siapa') && incomingMessages.includes('kamu')) {
                        await sock.sendMessage(
                            senderNumber,
                            { text: "Saya Brodi Bot WhatsApp yang dikembangkan oleh @e.2f.endi!" },
                            { quoted: messages[0] },
                            2000
                        );
                    } else {
